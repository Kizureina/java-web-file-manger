...
$('.modal-login form').addEventListener('submit', function(e){
  ...
  if(!/^\w{6,15}$/.test($('.modal-login input[name=password]').value)){
...
$('.modal-register form').addEventListener('submit', function(e){
  e.preventDefault()
  if(!/^\w{3,15}$/.test($('.modal-register input[name=username]').value)){
    $('.modal-register .errormsg').innerText = '用户名需输入3-15个字符，包括字母数字下划线'
    return false
  }
  ...
  if(!/^\w{6,15}$/.test($('.modal-register input[name=password]').value)){
